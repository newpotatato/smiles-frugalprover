"""
Trek A / step 3 (local, no GPU needed): analyze a pipeline run's artifacts.
...
def main():
    ...
    has_sc = all("sc_by_budget" in r for r in swept)
    single_acc, sc_acc = {}, {}
    for b in budgets:
        single_acc[b] = np.mean([r["p_by_budget"][str(b)] for r in swept])
        line = f"budget={b:>4}: single-sample acc={single_acc[b]:.3f}"
        if has_sc:
