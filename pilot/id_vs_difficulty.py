...
Works directly on the merged pilot_results.jsonl produced by merge_results.py,
which after the activations_only / full_sweep / baseline_single split mostly
contains activations_only records (phi + activations, no budget fields at
all) plus a smaller full_sweep tier that DOES have budget fields. Parts 1, 2
...
the solvability half of part 3 needs the full_sweep tier and is restricted to
it explicitly.
...
already collected -- no extra Colab time needed):
  ...
  3. Per-problem local density: unlike global ID estimators (one number per
     ...
     to the full_sweep tier, since that's the only one with outcome labels).

Run locally, after merge_results.py:
    python id_vs_difficulty.py pilot_results.jsonl
"""
import json
import sys
...
def main():
    ...
    # solvability (restricted to the full_sweep tier, which is the only one
    ...
    for layer in candidate_layers:
        ...
        if not swept_idx:
            print("  no full_sweep records -- skipping density-vs-solvability "
                  "(run RUN_MODE='full_sweep' on the labeling subset first)")
        else:
